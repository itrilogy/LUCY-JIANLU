...
const Footer: React.FC = () => {
  ...
  return (
    <>
      ...
      <footer
        className="mb-2 mt-8 w-full border-t border-emerald-900/10 bg-gradient-to-b from-transparent to-[#0D5E42]/[0.04] px-4 pt-8 dark:border-emerald-400/10 dark:to-[#0D5E42]/20"
        onClick={(e) => e.currentTarget.blur()}
      >
        <div className="mx-auto flex max-w-5xl flex-col items-center gap-4 text-center">
          {/* 实验室标识与阐述 */}
          <div className="flex flex-col items-center gap-2 sm:flex-row sm:gap-4">
            <img src={labLogo} alt="" className="h-14 w-auto object-contain opacity-95" aria-hidden />
            <div className="flex flex-col items-center sm:items-start">
              <div className="text-lg font-bold tracking-tight text-[#0D5E42] dark:text-[#3dba8a]">
                {BRAND.fullNameZh}
                <span className="ml-2 text-xs font-semibold tracking-[0.18em] text-[#0D5E42]/70 dark:text-[#3dba8a]/70">
                  {BRAND.labNameEn.toUpperCase()}
                </span>
              </div>
              <p className="mt-0.5 text-sm font-medium text-gray-600 dark:text-gray-300">{BRAND.slogan}</p>
              <p className="mt-0.5 text-xs tracking-wide text-gray-400 dark:text-gray-500">{BRAND.sloganEn}</p>
            </div>
          </div>
          ...
          <p className="max-w-xl text-[11px] leading-relaxed text-gray-400 dark:text-gray-500">
            产品 {BRAND.productNameZh}（{BRAND.productNameEn}）· {BRAND.productTagline} · {BRAND.fullNameZh} 出品
          </p>

          {/* 开源出处说明（短句，避免换行） */}
          <p className="px-3 text-[11px] leading-none text-gray-500 dark:text-gray-400 sm:whitespace-nowrap">
            基于开源项目{' '}
            <a
          ...
          {/* 原社媒链接行（内容不改动） */}
          <div className="mb-1 mt-1 flex w-full flex-wrap items-center justify-center gap-2.5 text-sm ease-in">
